...
void McmcObject::initial_param_values() {
    ...
    for (size_t parID=0; parID < m_parameter.size(); parID++) {
        if ( m_selectedParam[parID] ) {
            ...
            case 4:
                if ( m_lnormPrior[parID] ) { // sHigh_titer_one - LogNormal(0,sd)
                    m_parameter[parID] = rlnorm(m_gen, 0.0, m_sdLNormPrior[parID]);   
                } else {                // sHigh_titer_one - Uniform(0,5)
                    m_parameter[parID] = runif(m_gen, 0.0, 5.0);
                } 
                break;

            case 5:
                if ( m_lnormPrior[parID] ) { // sHigh_titer_two - LogNormal(0,sd)
                    m_parameter[parID] = rlnorm(m_gen, 0.0, m_sdLNormPrior[parID]);   
                } else {                // sHigh_titer_two - Uniform(0,5)
                    m_parameter[parID] = runif(m_gen, 0.0, 5.0);
                } 
                break;

            case 6:
                if ( m_lnormPrior[parID] ) { // sHigh_titer_three - LogNormal(0,sd)
                    m_parameter[parID] = rlnorm(m_gen, 0.0, m_sdLNormPrior[parID]);   
                } else {                // sHigh_titer_three - Uniform(0,5)
                    m_parameter[parID] = runif(m_gen, 0.0, 5.0);
                } 
                break;

            case 7:
                if ( m_lnormPrior[parID] ) { // iHigh_titer_one - LogNormal(0,sd)
                    m_parameter[parID] = rlnorm(m_gen, 0.0, m_sdLNormPrior[parID]);   
                } else {                // iHigh_titer_one - Uniform(0,5)
                    m_parameter[parID] = runif(m_gen, 0.0, 5.0);
                } 
                break;

            case 8:
                if ( m_lnormPrior[parID] ) { // iHigh_titer_two - LogNormal(0,sd)
                    m_parameter[parID] = rlnorm(m_gen, 0.0, m_sdLNormPrior[parID]);   
                } else {                // iHigh_titer_two - Uniform(0,5)
                    m_parameter[parID] = runif(m_gen, 0.0, 5.0);
                } 
                break;

            case 9:
                if ( m_lnormPrior[parID] ) { // iHigh_titer_three - LogNormal(0,sd)
                    m_parameter[parID] = rlnorm(m_gen, 0.0, m_sdLNormPrior[parID]);   
                } else {                // iHigh_titer_three - Uniform(0,5)
                    m_parameter[parID] = runif(m_gen, 0.0, 5.0);
                } 
                break;
