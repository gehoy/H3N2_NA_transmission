...
void Household::addIndividual(
  ...
  m_high_titer_sum.push_back(high_titer_sum);
...
double Household::log_pInf(
  ...
  int display
    ...
    //infectivity by categorical titer summary
    if (m_high_titer_sum[ind] == 1 && selectedParam[7] ) beta_i *= parameter[7];
    if (m_high_titer_sum[ind] == 2 && selectedParam[8] ) beta_i *= parameter[8];  
    if (m_high_titer_sum[ind] == 3 && selectedParam[9] ) beta_i *= parameter[9];
    ...
    // Susceptibility by categorical high vs low

    if (m_high_titer_sum[curr] == 1 && selectedParam[4] ) beta *= parameter[4];  
    if (m_high_titer_sum[curr] == 2 && selectedParam[5] ) beta *= parameter[5]; 
    if (m_high_titer_sum[curr] == 3 && selectedParam[6] ) beta *= parameter[6];
